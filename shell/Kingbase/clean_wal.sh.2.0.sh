...
cal_wal(){
    ...
    if [ "$result_dec" -lt 0 ]; then
        # 负数转换为32位无符号补码的大写形式
        result_hex=$(printf "%08X" $((result_dec + 0x100000000)))
    else
        # 正数补全前导零，大写输出
    ...
    # 显示计算过程和结果
    echo "===== 计算过程 ====="
    #echo "原始字符串: $hex_str (长度: $total_len)"
    #echo "前缀部分:   $prefix (长度: $prefix_len)"
    #echo "后8位:      $last8 (十六进制) = $last8_dec (十进制)"
    #echo "运算:       $last8_dec $op $num = $result_dec (十进制)"
    #echo "结果后8位:  $result_hex (十六进制)"
    echo "===== 最终结果 ====="
    echo "$final_result"

}

#cal_wal "000000010000000D000000F4" $OP $NUM
get_walfile(){
