...
#5.db list 
db_list(){
    DATABASES=$($bin_path/ksql  -Usystem -A -p $kingbase_port test -c "select datname as test  from sys_catalog.sys_database where datname not in('test','template1','template0','security');" | sed '$d')
}
...
do
    ...
    #循环创建扩展，若已存在报错后会继续执行
set +e
     $bin_path/ksql -Usystem -d $db -p $kingbase_port -c 'create extension passwordcheck;'
     $bin_path/ksql -Usystem -d $db -p $kingbase_port -c 'create extension identity_pwdexp;'
     $bin_path/ksql -Usystem -d $db -p $kingbase_port -c 'create extension sys_audlog;'
done
}
