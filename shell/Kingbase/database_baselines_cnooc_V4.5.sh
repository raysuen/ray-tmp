...
###
### Usage  :
###        ./optimize_database_baselines.sh [-u username]
###        -u: 指定数据库连接用户，默认值为system
###
### Reedit : Raysuen
### version: v3.0
...
### version:v3.5
###         reedit info: Function: GetPort,and using -p kingbase_port to connecting kingbase
### version:v4.5
###         reedit info: Support specify ksql user via -u parameter, default is system
####################################################################################################################
...
while getopts "u:" opt; do
  case $opt in
    ...
    \?)
      ...
      ;;
    :)
      echo "错误：选项 -$OPTARG 需要传入用户参数（例如：-u myuser）" >&2
      exit 1
      ;;
  esac
done
...
optimize_baselines(){
    ...
    if [ "${DBMode}" == "cluster" ];then
        ...
        kingbase_conf=$data_dir/kingbase.conf       #设置kingbase集群的配置文件
...
#edit param file sys_hba.conf
edit_hba_strust(){
    sed -i "s/^local   all/#local   all/g" ${data_dir}/sys_hba.conf
    sed -i '/#local   all.*/i local    all    all    trust' ${data_dir}/sys_hba.conf
    $bin_path/sys_ctl -D ${data_dir} reload
}

edit_hba_init(){
    sed -i '/^local    all    all    trust/d' ${data_dir}/sys_hba.conf
    sed -i "s/^#local   all/local   all/g" ${data_dir}/sys_hba.conf
    $bin_path/sys_ctl -D ${data_dir} reload
}
...
#5.db list 
db_list(){
    # ===================== 修改：替换硬编码的system为变量KSQL_USER =====================
    DATABASES=$($bin_path/ksql  -U${KSQL_USER} -A -p $kingbase_port test -c "select datname as test  from sys_catalog.sys_database where datname not in('test','template1','template0','security');" | sed '$d')
    # ================================================================================
}
...
set +e
    # ===================== 修改：替换硬编码的system为变量KSQL_USER =====================
     $bin_path/ksql -U${KSQL_USER} -d $db -p $kingbase_port -c 'create extension passwordcheck;'
     $bin_path/ksql -U${KSQL_USER} -d $db -p $kingbase_port -c 'create extension identity_pwdexp;'
     $bin_path/ksql -U${KSQL_USER} -d $db -p $kingbase_port -c 'create extension sys_audlog;'
    # ================================================================================
done
}
...
do
    ...
    if [ ${restart_option:-100} -eq 1 ]; then
        ...
        #edit sys_hba.conf to local trus
        edit_hba_strust
        #5. db list
        echo "5.db list"
        db_list  
        ...
        #edit sys_hba.conf to local init
        edit_hba_init
        break
